...
- **Timezone/offset behavior:** the request used `09:30:00`–`16:00:00` with
  no timezone qualifier (matching standard NYSE market hours, per
  `analyzer.py`'s own comment "ET timezone implied by API"). September 17,
  2026 falls within US Daylight Saving Time (EDT, UTC‑4), so a DST-aware
  conversion would be expected to resolve to `13:30`–`20:00` UTC. The data
  actually returned spans `14:30`–`21:00` UTC — a full hour later,
  consistent with a **fixed UTC‑5 (standard-time) offset** being applied
  regardless of the fact that the requested date is in daylight time. This
  directly reinforces the `improvements.md` Finding 1 (timezone ambiguity)
  with a concrete, measured example, though it does not by itself prove the
  internal mechanism (e.g., whether the API always uses a fixed offset or
  has some other DST-handling rule not exercised by this single window).
